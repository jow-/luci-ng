...
export class UciModelService {
  ...
  bindSelector(
    ...
    // validate selector
    if (selector.invalid) return throwError('Invalid selector');
  ...
  save(): void {
    ...
    for (const config of this.configs) {
      ...
      for (const section of sections) {
        ...
        if (param && param.values) calls.push(this._uci.set(param));
      }
      ...
      for (const section of sections) {
        calls.push(
          this._uci
            .addSection(section.getAddedParams()!)
            .pipe(tap(name => (section.name = section.oldName = name)))
        );
      }
    }
    ...
    forkJoin(calls)
      .pipe(debug('save'))
      .subscribe(d => console.log(d), e => console.log(e), () => console.log('complete'));
  }
}
